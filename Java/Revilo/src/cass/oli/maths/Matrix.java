...
final public class Matrix {
	...
	public void add(double factor) {
		for(int i = 0; i < rows; i++) {
			for(int j = 0; j < columns; j++) m[i][j] += factor;
		}
	}
	...
	public int[][] round(){
		...
		for(int i = 0; i < rows; i++) {
			for(int j = 0; j < columns; j++) A[i][j] = (int) Math.round(m[i][j]);
		}
		return A;
	}
	...
	public static Matrix add(Matrix A, Matrix B) {
		if(A.rows != B.rows || A.columns != B.columns) throw new IllegalArgumentException("Matrix sizes did not match.");
	...
	public static Matrix rotation(double theta) {
		return new Matrix(new Double[][] {{Math.cos(theta), -Math.sin(theta)}, {Math.sin(theta), Math.cos(theta)}});
	}
}
